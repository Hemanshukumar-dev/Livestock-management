...
'Healthy' => 'bg-emerald-400/20 text-emerald-100 border-emerald-400/40',
                            ...
                            ][$livestock->health_status] ?? 'bg-white/10 text-white/80 border-white/20';
                        $statusDot = [
'Healthy' => 'bg-emerald-300',
                            ...
                            ][$livestock->health_status] ?? 'bg-white/60';
                    ?>
                ...
                <div class="space-y-6">
                    @foreach ($livestock->histories as $history)
                        <?php
                            ...
                            $eventColor = $eventColorMap[$history->event_type] ?? ['bg-slate-100 text-slate-600 border-slate-200', 'bg-slate-400', '📝'];
                        ?>
                                ...
                                <div class="mt-3 flex items-center justify-between">
                                    ...
                                    @if ($currentUser?->isAdmin() || ($currentUser?->isOwner() && $livestock->owner?->user_id === $currentUser->id))
                                        <div class="flex items-center gap-2">
                                            <a href="{{ route('livestock.histories.edit', $history->id) }}" class="rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50 transition">✏️ Edit</a>
            ...
            <form method="POST" action="{{ route('livestock.histories.store', $livestock->id) }}" class="space-y-4">
                ...
                <div class="grid gap-4 sm:grid-cols-3">
                    <div>
                        ...
                        <select name="event_type" required class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20">
                    ...
                    <div>
                        ...
                        <input type="date" name="event_date" required class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20">
                    </div>
                    <div>
                        <label class="mb-1 block text-xs font-medium uppercase tracking-[0.15em] text-slate-500">Description</label>
                        <textarea name="description" rows="1" placeholder="Short details (optional)" class="w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-500/20"></textarea>
                    </div>
                </div>
    ...
    {{-- Back Link --}}
    <div class="mt-6">
        <a href="{{ route('livestock.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-slate-500 transition hover:text-sky-700">
    ...
    <div id="delete-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/40 backdrop-blur-sm">
        <div class="mx-4 w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div class="text-center">
                ...
                <p class="mt-1 text-sm text-slate-600">Are you sure you want to delete <strong>{{ $livestock->tag_number }}</strong>? This action cannot be undone. All health history for this animal will also be removed.</p>
            </div>
    ...
    <div id="delete-history-modal" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-900/40 backdrop-blur-sm">
        <div class="mx-4 w-full max-w-sm rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl">
            <div class="text-center">
                <div class="mx-auto flex h-12 w-12 items-center justify-center rounded-full bg-red-100 text-2xl">⚠️</div>
                <h3 class="mt-4 text-lg font-bold text-slate-900">Delete History Record</h3>
                <p class="mt-1 text-sm text-slate-600">Are you sure you want to delete the <strong id="delete-history-type"></strong> record? This action cannot be undone.</p>
            </div>
            <div class="mt-6 flex gap-3">
                <button
                    type="button"
