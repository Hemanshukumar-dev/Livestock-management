...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        <a href="{{ route('schemes.index') }}" class="inline-flex items-center text-sm font-semibold text-slate-500 hover:text-sky-700">
            ← Back to Schemes
        </a>
        @if (auth()->user()?->isAdmin())
            <div class="flex gap-2">
                <a href="{{ route('schemes.edit', $scheme) }}" class="rounded-lg border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Edit Scheme</a>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <span class="text-sm font-semibold text-slate-500">{{ $scheme->category }}</span>
                </div>
                
                <h1 class="text-2xl sm:text-3xl font-bold text-slate-900 leading-tight mb-4">{{ $scheme->title }}</h1>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                ...
                <ul class="space-y-4 text-sm">
                    <li class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-slate-600">{{ $scheme->animal_type ?? 'All Animals (Generic)' }}</p>
                        </div>
                    </li>
                    <li class="flex items-start gap-3">
                        <span class="text-slate-400 mt-0.5">📅</span>
                ...
                @if ($scheme->apply_link)
                    <div class="mt-6 pt-6 border-t border-slate-100">
                        <a href="{{ $scheme->apply_link }}" target="_blank" rel="noopener noreferrer" class="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-800">
